...
var MCStream = exports.MCStream = function MCStream(protocolParser, c) {
  ...
  this.keepaliveInterval = setInterval(function() {
    that.write('keepalive');
  }, 2000);
  this.clearstream.on('close', function() {
...
MCStream.prototype.parse = function parse() {
  ...
  if (parseResult.bytesLeft != buffers[0].length) {
    var oldLength = buffers[0].length;
    buffers[0] = buffers[0].slice(oldLength - parseResult.bytesLeft,
                                  oldLength);

    if (buffers[0].length == 0) buffers.shift();

    this.buffersTotal -= oldLength - parseResult.bytesLeft;

    for (var i = 0, len = parseResult.length; i < len; i++) {
      this.emit('packet', parseResult[i]);
    };
  }
